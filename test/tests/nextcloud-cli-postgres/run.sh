...
dbName="test_${RANDOM}_db" # dbName has to be set to something that does not require escaping: https://github.com/khulnasoft/docker-images/pull/6252#issuecomment-520095703
...
# NEXTCLOUD_ADMIN_USER has to be set to something that does not require escaping: https://github.com/khulnasoft/docker-images/pull/6252#issuecomment-520095703
cid="$(docker run -d \
	--link "$dbCid":db \
	-e POSTGRES_HOST='db' \
	-e POSTGRES_USER="$dbUsr" \
